...
public class Visitor extends AJmmVisitor<Void, Void> {
    ...
    private Void visitVarDec(JmmNode jmmNode, Void unused) {
        ...
        switch (jmmNode.getJmmParent().getKind()) {
            case "ClassDeclaration" ->
                    symbolTable.getFields().add(new Symbol(new Type(typesymb, jmmNode.getJmmChild(0).getKind().equals("Array")), namesymb));
            case "MethodDeclaration" -> {
                var methodName = jmmNode.getJmmParent().get("methodName");
                symbolTable.methvars.putIfAbsent(methodName, new ArrayList<>());
                symbolTable.getLocalVariables(methodName).add(new Symbol(new Type(typesymb, jmmNode.getJmmChild(0).getKind().equals("Array")), namesymb));
            }
        }
        return null;
    }
    ...
    private Void visitInstanceMethod(JmmNode jmmNode, Void unused) {
        ...
        var returnType = jmmNode.getJmmChild(0).get("value");
        var returnTypeIsArray = jmmNode.getJmmChild(0).getKind().equals("Array");
        ...
        for(int i = 1; i < jmmNode.getNumChildren(); i++) {
            ...
            symbolTable.methparams.get(methodName).add(new Symbol(new Type(paramType, methodParam.getJmmChild(0).getKind().equals("Array")), paramName));
        }
        return null;
    }
